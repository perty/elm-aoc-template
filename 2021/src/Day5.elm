module Day5 exposing (VentLine, endPointParser, equation, foldl, main, parse, planParser, puzzleInput, solution1, solution2, solve1, solve2)
...
pointsOnLine ventLine =
    if ventLine.x1 == ventLine.x2 then
        let
            ys =
                List.range (Basics.min ventLine.y1 ventLine.y2) (Basics.max ventLine.y1 ventLine.y2)
        in
        List.map (\y -> { x = ventLine.x1, y = y }) ys
